...
import { addCategory, updateCategory } from "@/src/API/CategoriesAPI";
import { useRouter } from "next/navigation";

export default function EditCategory({ category }) {
  const [name, setName] = useState(category.name);
  const [description, setDecription] = useState(category.description);
  const [slug, setSlug] = useState(category.urlSlug);
  ...
  const handleSubmit = async (event) => {
    ...
    const res = updateCategory(category.id, data);
  ...
  return (
    <div>
      <SectionHeader title={"Chỉnh sửa danh mục"} />
      <form method="put" className="mt-5 max-w-lg" onSubmit={handleSubmit}>
